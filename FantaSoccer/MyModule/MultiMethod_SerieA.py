...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 22 09:38:46 2021
...
if(source=="Fantasoccer"):
    columns_list = ['Voto FS','Gol_segnati_fs', 'Gol_subiti_fs','Ammonizione', 'Espulsione', 'Rigori_segnati', 'Rigori_sbagliati','Rigori_parati']
if(source=="Mediaset"):
    columns_list = ['Voto DS_SM','Gol_segnati_fs', 'Gol_subiti_fs','Ammonizione', 'Espulsione', 'Rigori_segnati', 'Rigori_sbagliati','Rigori_parati']

pl_file = {}

for c in columns_list:
    print(c)
    pl_file[c] = PMV.CreatePlayerFileVar(giorn,league,c,source)
    
if(source=="Mediaset"):
    pl_file['Voto FS'] = pl_file['Voto DS_SM']
...
seriea_lineups = pd.read_excel(league+'/Lineups_'+str(giorn)+'.xlsx')
seriea_lineups = seriea_lineups.drop(columns=['Unnamed: 0'])
seriea_odds = pd.read_excel(league+'/Odds_giornata_'+str(giorn)+'.xlsx')
seriea_odds = seriea_odds.drop(columns=['Unnamed: 0'])
